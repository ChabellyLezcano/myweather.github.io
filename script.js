$(document).ready(function () {
    ...
    function searchWeather(queryURL) {
      ...
      $.ajax({
        ...
        let formattedDate = moment.unix(response.dt).format("DD/MM/YY"); // Cambio de formato de fecha
        ...
        let temperatureCelsius = (response.main.temp - 273.15).toFixed(1);
        temperatureEl.html(temperatureCelsius);
  
        humidityEl.text(response.main.humidity);
        windEl.text((response.wind.speed * 3.6).toFixed(1));
        ...
        $.ajax({
          ...
          for (let i = 0; i <= 5; i++) {
            ...
            let formattedDate = moment.unix(currDay.dt).format("ddd D MMM");
  
            $(`div.day-${i} .card-title`).text(formattedDate);
            ...
            let temperatureCelsius = (currDay.temp.day - 273.15).toFixed(1);
            $(`div.day-${i} .fiveDay-temp`).text(temperatureCelsius);
